...
/**
 * Allows components, preferably horizontal flowing components such as boxes,
 * to be layed and and reordered.
 *
...
public class ComponentTable extends ModernComponent implements Iterable<Component> {
	...
	private static class ComponentList extends ModernComponent implements Iterable<Component> {
		...
		private ModernCheckBox mCheckAll = 
				new ModernCheckBox(UI.MENU_SELECT_ALL);
	...
	private static class CContainer extends ModernComponent implements ModernClickListener, ModernClickEventProducer {
		...
		private ModernCheckBox mCheckSelected = 
				new ModernCheckBox();
	...
	/** The m order map. */
	private Map<Integer, CContainer> mOrderMap =
			new HashMap<Integer, CContainer>();
	
	/** The m C map. */
	private Map<CContainer, Integer> mCMap =
			new HashMap<CContainer, Integer>();
	
	/** The m box. */
	private ComponentList mBox = new ComponentList();
	
	/** The m move up button. */
	private ModernButton mMoveUpButton = 
			new ModernButton(UIService.getInstance().loadIcon(ArrowUpVectorIcon.class, 16));
	
	/** The m move down button. */
	private ModernButton mMoveDownButton = 
