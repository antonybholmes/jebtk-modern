...
/**
 * Tree for checkable nodes that will respond to mouse events and toggle
 * whether nodes are checked or not.
 *
...
public class ModernCheckTree<T> extends ModernTree<T> {
	...
	public List<CheckTreeNode<T>> getCheckedNodes() {
		...
		List<CheckTreeNode<T>> ret = 
				new ArrayList<CheckTreeNode<T>>(nodes.size());
	...
	/**
	 * Returns a list of nodes in the tree that are check nodes. Nodes are
	 * in order of how they are found in the tree exploring depth first.
	 *
